...
class Barangmasuk extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('idbarang, jumlah, keterangan, tanggal', 'required'),
			array('idbarang, jumlah', 'numerical', 'integerOnly'=>true),
			array('keterangan', 'length', 'max'=>25),
			array('tanggal', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('idmasuk, idbarang, tanggal, jumlah, keterangan', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('idmasuk',$this->idmasuk);
