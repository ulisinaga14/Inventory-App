...
class BarangkeluarController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if (isset($_POST['Barangkeluar'])) {
			$idBarang = $_POST['Barangkeluar']['idbarang'];

			$barangKeluarModel->attributes = $_POST['Barangkeluar'];
			$inputJumlah = $_POST['Barangkeluar']['jumlah'];

			$stockBarang = Stock::model()->findByPk($idBarang);
			$stockBarang->stock = intval($stockBarang->stock) - intval($inputJumlah);

			if ($barangKeluarModel->save() && $stockBarang->save(false))
				$this->redirect(array('view', 'id' => $barangKeluarModel->idkeluar));
		}

		$this->render('create', array(
			'model' => $barangKeluarModel,
		));
	}
	...
	public function actionUpdate($id)
	{
		...
		if (isset($_POST['Barangkeluar'])) {
			...
			if ($model->save())
				$this->redirect(array('view', 'id' => $model->idkeluar));
		}
	...
	{
		if (isset($_POST['ajax']) && $_POST['ajax'] === 'barangkeluar-form') {
